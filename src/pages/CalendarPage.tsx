  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="rounded-3xl border border-gray-300 bg-white p-3 shadow-sm dark:border-gray-700 dark:bg-gray-900 sm:p-4">
          ...
          <div className="flex min-w-0 items-center gap-1">
            ...
            >
              {Array.from(
                {
                  length:
                    Math.max(currentYear, calendarStartYear) +
                    2 -
                    calendarStartYear,
                },
            ...
            <button
              onClick={() => jumpToMonth(jumpYear, jumpMonth)}
        ...
        <div className="grid grid-cols-7 gap-y-1 gap-x-0 sm:gap-1">
          {days.map((day) => {
            ...
            const mobileEvents = [
              dayEvents.find(
                (event) => event.createdBy === "Remus" && !event.isRomantic,
              ),
              dayEvents.find(
                (event) => event.createdBy === "Nicole" && !event.isRomantic,
              ),
            ...
            return (
              <button
                ...
                className={`relative min-h-16 rounded-xl border p-1 text-left transition-all sm:min-h-16 ${hasMultiDayEvent ? "z-10" : "z-0"} ${
                  isSelected
                    ? "border-gray-900 bg-gray-100 dark:border-gray-100 dark:bg-gray-800"
                    : "border-gray-200 hover:border-gray-400 dark:border-gray-700 dark:hover:border-gray-500"
                } ${day.getDay() === 0 ? "bg-red-50/60 dark:bg-red-950/20" : day.getDay() === 6 ? "bg-blue-50/60 dark:bg-blue-950/20" : ""} ${!inMonth ? "opacity-35" : ""}`}
              >
                <div
                ...
                <div className="mt-1 h-10 overflow-visible sm:h-auto sm:min-h-8">
                  <div
                  ...
                  <div className="flex h-6 flex-row items-center justify-start gap-1 overflow-visible sm:hidden">
                    {mobileEvents.map((event) => (
                      event.startDate !== event.endDate ? (
                        <span
                          key={event.id}
                          aria-label={`${getCalendarEventTitle(event, dateKey)}（跨日活動）`}
                          className={`relative z-20 -mx-1 block h-2 ${getEventBarWidth(event, dateKey)} ${getEventBarStyle(event)} ${getEventBarRadius(event, dateKey)}`}
                        />
                      ) : (
                        <span
                          key={event.id}
                    ...
                    {hasRomanticEvent && <span aria-label="紀念日">💕</span>}
                  </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-[60] flex items-end justify-center overflow-x-hidden overflow-y-hidden bg-black/40 p-3 sm:items-center sm:p-4">
          <form
